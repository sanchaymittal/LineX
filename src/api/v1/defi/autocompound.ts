  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!defiServices || !defiServices.autoCompoundVault) {
        logger.warn(
          `⚠️ AutoCompound service not available, using fallback mock data for ${address}`
        );
      ...
      try {
        ...
        const preview = {
          ...
          compoundInfo: {
            ...
            nextCompound:
              (vaultInfo.lastCompound || Date.now()) +
              parseInt(vaultInfo.compoundingRate || '24') * 3600000,
          ...
          risks: {
            ...
            factors: [
              'Auto-compound smart contract risk',
              'Yield strategy risk',
              'Market volatility',
              'Compound frequency risk',
            ],
          },
          timestamp: Date.now(),
        };
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      try {
        ...
        const baseExpectedAssets =
          totalSupply > 0 ? (sharesBN * totalAssets) / totalSupply : BigInt(0);
        ...
        const compoundEarnings =
          expectedAssets -
          (originalShares * totalAssets) / (totalSupply > 0 ? totalSupply : BigInt(1));
